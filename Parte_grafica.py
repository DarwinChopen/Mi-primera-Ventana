...
def sumar():
    try:
        ...
        resultado = num1 + num2
        etiqueta_resutado.config(text=f"Suma:{resultado}")
    except ValueError:
...
def restar():
    try:
        ...
        num2 = int(entrada2.get())
        resultado = num1 - num2
        etiqueta_resutado.config(text=f"Resta:{resultado}")
    except ValueError:
        etiqueta_resutado.config(text="Por favor, ingresa números válidos.")
...
def multiplicar():
    try:
        ...
        num2 = int(entrada2.get())
        resultado = num1 * num2
        etiqueta_resutado.config(text=f"Multiplicacion: {resultado}")
    except ValueError:
        etiqueta_resutado.config(text="Por favor, ingresa números válidos.")
...
def dividir():
    try:
        ...
        num2 = int(entrada2.get())
        resultado = num1 / num2
        etiqueta_resutado.config(text=f"Division: {resultado}")
    except ValueError:
        etiqueta_resutado.config(text="Por favor, ingresa números válidos.")
...
def limpiar():
    ...
    etiqueta_resutado.config(text="")

frame_botones = tk.Frame(ventana, bg="grey",)
frame_botones.pack(pady=5)

boton_sumar = tk.Button(frame_botones, text="+",width=6, height=2, command=sumar)
boton_sumar.pack(side="left",padx=5)

boton_restar = tk.Button(frame_botones, text="-",width=6, height=2, command=restar)
boton_restar.pack(side="left",padx=5)

boton_multiplicar = tk.Button(frame_botones, text="*",width=6, height=2, command=multiplicar)
boton_multiplicar.pack(side="left",padx=5)

boton_dividir = tk.Button(frame_botones, text="/",width=6, height=2, command=dividir)
boton_dividir.pack(side="left",padx=5)

etiqueta_resutado = tk.Label(ventana, text="00")
